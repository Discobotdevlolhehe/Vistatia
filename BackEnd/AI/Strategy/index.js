...
async function generateStrategy(user, prompt) {
  let mind = suckMemory();

  // sanity check or what's left of it
  if (!mind[user]) {
    mind[user] = { strategies: [] };
  } else if (!Array.isArray(mind[user].strategies)) {
    mind[user].strategies = [];
  }
  ...
  mind[user].strategies.push({ prompt: prompt, strategy: txt });
  yeetMemory(mind);
