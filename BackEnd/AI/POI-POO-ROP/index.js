...
async function generatePoiPooRop(user, prompt) {
  let mind = suckMemory();

  // sanity check or what's left of it
  if (!mind[user]) {
    mind[user] = { poispoosrops: [] };
  } else if (!Array.isArray(mind[user].poispoosrops)) {
    mind[user].poispoosrops = [];
  }
  ...
  mind[user].poispoosrops.push({ prompt: prompt, poispoosrop: txt });
  yeetMemory(mind);
...
module.exports = {
  generatePoiPooRop: generatePoiPooRop
};
